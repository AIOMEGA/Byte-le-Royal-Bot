...
class Client(UserClient):
    ...
    def take_turn(self, turn, actions, world, avatar):
        ...
        if current_tile.occupied_by.object_type == self.my_station_type:
            ...
            self.current_state = State.MINING
            #If the current position is the base, move accordingly and switch to mining
            if avatar.position == self.base_position:
                countATech=0
                if self.my_station_type == ObjectType.TURING_STATION:
                    actions = [ActionType.MOVE_RIGHT]
                elif self.my_station_type == ObjectType.CHURCH_STATION:
